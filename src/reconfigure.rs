...
pub fn apply_changes(config: &mut Config, changes: &[(String, String)]) {
    for (key, value) in changes {
        match key.as_str() {
            ...
            "dns.dot_listen" => {
                config.dns.dot_listen = if value.is_empty() {
                    None
                } else {
            ...
            "dns.doh_listen" => {
                config.dns.doh_listen = if value.is_empty() {
                    None
                } else {
                    Some(vec![value.clone()])
                };
            ...
            "dns.doq_listen" => {
                config.dns.doq_listen = if value.is_empty() {
                    None
                } else {
                    Some(vec![value.clone()])
                };
...
mod tests {
    ...
    fn resolved_disable_for_port53() {
        assert_eq!(
            needs_resolved_change("0.0.0.0:5353", "0.0.0.0:53"),
            ResolvedAction::Disable
        );
    }
    ...
    fn resolved_enable_leaving_port53() {
        assert_eq!(
            needs_resolved_change("0.0.0.0:53", "0.0.0.0:5353"),
            ResolvedAction::Enable
        );
    }
    ...
    fn resolved_none_for_non53() {
        assert_eq!(
            needs_resolved_change("0.0.0.0:5353", "0.0.0.0:8053"),
            ResolvedAction::None
        );
    }
    ...
    fn resolved_none_same_port53() {
        assert_eq!(
            needs_resolved_change("0.0.0.0:53", "127.0.0.1:53"),
            ResolvedAction::None
        );
    }
